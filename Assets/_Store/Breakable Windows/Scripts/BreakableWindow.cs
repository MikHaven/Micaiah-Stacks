...
public class BreakableWindow : MonoBehaviour
{
    ...
    [SerializeField] List<GameObject> splinters;
    Vector3[] vertices;
    Vector3[] normals;

    bool allreadyCalculated = false;
    GameObject splinterParent;
    int[] tris;
    ...
    {
        splinters.RemoveAll(item => item == null);
        if (isBroken == false)
        {
            //if (transform.rotation.eulerAngles.x != 0 || transform.rotation.eulerAngles.z != 0)
            if (transform.localRotation.eulerAngles.x != 0 || transform.localRotation.eulerAngles.z != 0)
            {
                Destroy(gameObject);
                Debug.LogWarning("Warning: Window must not be rotated around x and z!");
            }
    ...
    void DoShatterWindow()
    {
        if (allreadyCalculated == true)
        {
            ...
            if (addTorques)
            {
                ...
                {
                    Rigidbody spliterRB = splinters[i].GetComponent<Rigidbody>();
                    if (spliterRB != null)
                    {
                        spliterRB.AddTorque(new Vector3(Random.value > 0.5f ? Random.value * 50 : -Random.value * 50, Random.value > 0.5f ? Random.value * 50 : -Random.value * 50, Random.value > 0.5f ? Random.value * 50 : -Random.value * 50));
                        Destroy(spliterRB, 10f);
                    }
                }
            }
        }
